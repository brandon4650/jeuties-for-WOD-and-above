...
namespace JeutieControl
{
  public class Form1 : Form
  {
    ...
    private ToolStripMenuItem openbnetserverConfigToolStripMenuItem;
    ...
    private void InitializeComponent()
    {
      ...
      this.openbnetserverConfigToolStripMenuItem = new ToolStripMenuItem();
      ...
      this.fileToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[4]
      {
        ...
        (ToolStripItem) this.openbnetserverConfigToolStripMenuItem,
      ...
      this.openbnetserverConfigToolStripMenuItem.Name = "openbnetserverConfigToolStripMenuItem";
      this.openbnetserverConfigToolStripMenuItem.Size = new Size(208, 22);
      this.openbnetserverConfigToolStripMenuItem.Text = "Open bnetserver Config";
      this.openbnetserverConfigToolStripMenuItem.Click += new EventHandler(this.openbnetserverConfigToolStripMenuItem_Click);
    ...
    private void Form1_Load(object sender, EventArgs e)
    {
      ...
      this.bnetserverRestartCount = 0U;
      this.worldServerRestartCount = 0U;
      this.bnetserverInfo = new ProcessStartInfo("bnetserver.exe");
      ...
      this.bnetserver.EnableRaisingEvents = true;
      this.bnetserver.Exited += new EventHandler(this.autoRestartbnetserver);
      this.CheckPaths();
    }
    ...
    private void autoRestartbnetserver(object sender, EventArgs e)
    {
      if (this.AutoRestart.Checked)
      {
        if (!this.isServerPathCorrect)
          return;
        this.bnetserver.Start();
        ++this.bnetserverRestartCount;
    ...
    {
      ServerFolderBrowser.Description = "The server path needs to be pointed to the core folder in which the bnetserver and Worldserver applications reside.";
    ...
    {
      if (File.Exists(this.ServerPath.Text + "\\bnetserver.exe") && File.Exists(this.ServerPath.Text + "\\worldserver.exe"))
      {
        ...
        this.bnetserverInfo.WorkingDirectory = this.ServerPath.Text;
      }
      else
      {
    ...
    private void stopAuth_Click(object sender, EventArgs e)
    {
      try
      {
        if (this.bnetserver.HasExited)
          return;
        this.bnetserver.CloseMainWindow();
      }
      catch (InvalidOperationException)
      {
      }
    ...
    private void openbnetserverConfigToolStripMenuItem_Click(object sender, EventArgs e)
    {
      if (this.isServerPathCorrect)
        Process.Start("notepad.exe", this.ServerPath.Text + "\\bnetserver.conf");
      else
        this.Log("Error!  Can't open the config file, incorrect server path.");
    }
